  ...
  return `
    <header id="site-header" class="site-header sticky top-0 z-50 px-4 py-4 transition-all duration-300" data-reveal>
      <div class="nav-shell mx-auto flex max-w-7xl items-center justify-between gap-3 rounded-full px-4 py-3 transition-all duration-300 md:gap-4 md:px-5">
        ...
        <nav class="hidden min-w-0 flex-1 items-center justify-center gap-1 rounded-full bg-white/35 px-2 py-1 md:flex" aria-label="${t('nav.primary')}">
        ...
        <div class="nav-actions hidden shrink-0 items-center justify-end gap-2 md:flex">
          ${iconButton(t('nav.searchProducts'), searchIcon, '', 'data-search-open aria-haspopup="dialog"')}
          ${ThemeToggle()}
          ${LanguageSwitcher()}
          <a href="${window.location.pathname === '/' ? '#newsletter' : '/#newsletter'}" class="nav-login ripple-button rounded-full bg-[#0d3b2e] px-5 py-2.5 text-sm font-semibold text-white shadow-[0_12px_28px_rgba(13,59,46,0.22)] outline-none transition hover:bg-[#143f31] focus-visible:ring-2 focus-visible:ring-brand-gold/70 focus-visible:ring-offset-2">${t('buttons.login')}</a>
          <a href="/wishlist" class="nav-icon-button ripple-button relative" aria-label="${t('nav.openWishlist')}">
            ${heartIcon}
            <span class="wishlist-badge absolute -right-1 -top-1 grid h-5 min-w-5 place-items-center rounded-full bg-brand-peach px-1 text-[10px] font-bold text-[#2f2419] shadow-sm" aria-label="${t('nav.wishlistCount', { count: wishlistCount })}" data-wishlist-count>${wishlistCount}</span>
          </a>
          <a href="/cart" class="nav-icon-button ripple-button relative" aria-label="${t('nav.openCart')}">
            ${cartIcon}
            <span class="cart-badge absolute -right-1 -top-1 grid h-5 min-w-5 place-items-center rounded-full bg-brand-gold px-1 text-[10px] font-bold text-[#2f2419] shadow-sm" aria-label="${t('nav.cartCount', { count: cartQuantity })}" data-cart-count>${cartQuantity}</span>
          </a>
        </div>

        <button id="menu-toggle" type="button" class="nav-icon-button ripple-button ml-auto md:hidden" aria-label="${t('nav.openMenu')}" aria-controls="mobile-drawer" aria-expanded="false">
      ...
      <aside id="mobile-drawer" class="mobile-drawer fixed right-0 top-0 z-50 h-dvh w-[min(86vw,24rem)] translate-x-full bg-white/92 p-5 shadow-[-24px_0_70px_rgba(31,23,16,0.2)] backdrop-blur-2xl transition duration-300 md:hidden" role="dialog" aria-modal="true" aria-label="${t('nav.mobile')}" aria-hidden="true" inert>
        <div class="flex items-center justify-between">
          ...
          <button id="menu-close" type="button" class="nav-icon-button ripple-button" aria-label="${t('nav.closeMenu')}">
            <svg viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.9" aria-hidden="true">
        ...
        <nav class="mt-10 grid gap-2" aria-label="${t('nav.mobilePrimary')}">
